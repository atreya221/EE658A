%% Example 2 - Truck Backer-Upper Problem (using Numerical Fuzzy approach)
...
b = 4;                      % length of truck
%% 
% Now, we simulate for 80 iterations all the test inputs and plot their trajectories.
...
for j=1:3
    ...
    for itr=1:iter
        ...
        theta_test_deg = x_test_deg( ...
            reduced_rules_with_deg(:,1)).*phi_test_deg( ...
            reduced_rules_with_deg(:,2));
...
saveas(gcf,'trucktrajectory2.png');
%% 
% We see that the trajectories of neither of the trucks converge to the loading 
% dock state. Hence, the fuzzy rules obtained from the truncated data are not 
% sufficient for driving the truck to the loading dock.
%